...
bool lookup(string s)
{
    ...
    {
        int mid = (min + max) / 2;
        int comparison_result = strcmp(s, dictionary.words[mid].letters);
...
void scramble(void)
{
    // Only need to go halfway because the swapping is mirrored across the halfway point
    for (int i = 0; i < DIMENSION / 2; i++)
    {
        // The layer to rotate CW
        for (int j = i; j < DIMENSION - i - 1; j++)
        {
            // Rotate the 4 "corners"
            char temp = grid[i][j];
            grid[i][j] = grid[DIMENSION - 1 - j][i];
            grid[DIMENSION - 1 - j][i] = grid[DIMENSION - 1 - i][DIMENSION - 1 - j];
            grid[DIMENSION - 1 - i][DIMENSION - 1 - j] = grid[j][DIMENSION - 1 - i];
